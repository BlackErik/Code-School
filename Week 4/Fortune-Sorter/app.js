...
Vue.component("new-fortune", {
  ...
  <div>
    ...
    <button @click="sortFortune(newFortune,0)"> Good </button>
    <button @click="sortFortune(newFortune,1)"> Bad </button>
  </div>
  `,
  props: [
    "newFortune",
...
Vue.component("sorted-fortune", {
  ...
  template: `
  <div>
    <h4 v-bind:style="{color: color}"> {{fortune}} </h4>
  </div>
  `,
  props: [
    "fortune",
...
var app = new Vue({
  ...
  methods: {
    ...
    sortFortune: function (fortune, category) {
      let goodfortunes = this.goodFortuneList;
      let badfortunes = this.badFortuneList;
      if (category == 0) {
        goodfortunes.push(fortune);
      } else {
        badfortunes.push(fortune);
      }
      this.getNewFortune();
    },
  },
